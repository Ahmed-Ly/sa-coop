...
void CClientNetwork::BulletSyncEvent(librg_message_t* msg)
{
	...
	if (shooterPlayer && shooterPlayer != LocalPlayer())
	{
		...
		CHooks::DoBulletImpact(&shooterPlayer->m_aWeapons[shooterPlayer->m_nActiveWeaponSlot], shooterPlayer, hitEntity, &bsData.start, &bsData.end, &bsData.colPoint, bsData.ahead);
	}
}
...
void CClientNetwork::on_entity_update(librg_event_t *event) 
{
	...
	if (event->entity->type == VCOOP_PLAYER)
	{
		...
		if(!spd.isInVehicle) ped->m_placement.m_vPosn = (*(CVector *)&event->entity->position);
	...
	else if(event->entity->type == VCOOP_PED)
	{
		...
		ped->m_placement.m_vPosn = (*(CVector *)&event->entity->position);
	...
	else if (event->entity->type == VCOOP_VEHICLE)
	{
		...
		veh->m_placement.m_vPosn = (*(CVector *)&event->entity->position);
