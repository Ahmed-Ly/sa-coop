...
char __fastcall CPlayerPed__ProcessControl_Hook(CPlayerPed * This, DWORD _EDX)
{
	if (This != LocalPlayer())
	{
		...
		*CPad::GetPad(0) = gGame->remotePlayerKeys[currentPlayerID];
		...
		localPlayerCameraMode = (BYTE)TheCamera.m_aCams[TheCamera.m_nActiveCam].m_nMode;

		// onfoot mouse looking mode.
		TheCamera.m_aCams[TheCamera.m_nActiveCam].m_nMode = (eCamMode)4;

		// save local player's aim
		localPlayerLookFrontX = *(CAMERA_AIM*)&TheCamera.m_aCams[TheCamera.m_nActiveCam].m_vecFront;

		// set remote player's aim
		*(CAMERA_AIM*)&TheCamera.m_aCams[TheCamera.m_nActiveCam].m_vecFront = gGame->remotePlayerLookFrontX[currentPlayerID];

		// call the internal CPlayerPed[]::Process
		original_CPlayerPed__ProcessControl(This);

		// restore the camera mode.
		TheCamera.m_aCams[TheCamera.m_nActiveCam].m_nMode = (eCamMode)localPlayerCameraMode;
		...
		*(CAMERA_AIM*)&TheCamera.m_aCams[TheCamera.m_nActiveCam].m_vecFront = localPlayerLookFrontX;
		return 0;
	}
