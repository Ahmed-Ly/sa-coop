...
public:
	...
	DeveloperConsole()
	{
		...
		AddLog("[DeveloperConsole] Initialized");
	}
	~DeveloperConsole()
	{
		ClearLog();
	...
	void    Draw(bool* p_open)
	{
		...
		for (int i = 0; i < Items.Size; i++)
		{
			...
			if (strstr(item, "[error]") || strstr(item, "[ERROR]")) col = ImColor(1.0f, 0.4f, 0.4f, 1.0f);
	...
	void    ExecCommand(const char* command_line)
	{
		...
		else if (Stricmp(command_line, "COLLAPSE") == 0)
		{
			Collapsed = !Collapsed;
		}
...
#endif
class CDebugScreen : public CRenderTemplate
{
public:
	CDebugScreen();
	~CDebugScreen();
